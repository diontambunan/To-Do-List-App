...
# GUI CODES
window = tk.Tk()
window.title("To Do List | by Kelompok 2")

top_left = tk.Frame(window)
top_right = tk.Frame(window)


list_notes = Listbox(top_left, height=35, width=25, font = "Calibri 14")
...
note_title = tk.Entry(text_frame, width=52, font = "Calibri 13")
note_title.insert(tk.END, "Judul")
...
note_text = tk.Text(text_frame, height=34, width=53, font = "Calibri 13")
...
def save_note():
    ...
    if len(title) < 1:
        tk.messagebox.showerror(title="ERROR!!!", message="Kamu HARUS masukkan judul note")
        return

    note = note_text.get("1.0", tk.END)
    if len(note.rstrip()) < 1:
        tk.messagebox.showerror(title="ERROR!!!", message="Kamu HARUS masukkan isi dari note")
        return
    ...
    if title_exist is True:
        tk.messagebox.showerror(title="ERROR!!!", message="Judul note telah ada. Gunakan judul baru")
...
def update_note():
    ...
    note = note_text.get("1.0", tk.END)
    if len(note.rstrip()) < 1:
        tk.messagebox.showerror(title="ERROR!!!", message="Kamu HARUS masukkan isi note")
        return
...
def delete_note():
    ...
    if len(title) < 1 or len(notes.rstrip()) < 1:
        tk.messagebox.showerror(title="ERROR!!!", message="Pilih note yang mau dihapus")
        return

    result = tk.messagebox.askquestion("Delete", "Kamu yakin ingin menghapus?", icon='warning')
